...
def tri_cen_loss(ft, lbl):
    loss = 0
    N = ft.size(0)
    n_class = lbl.max().item() + 1
    c_center = []
    for l in range(n_class):
        c_center.append(ft[lbl==l].mean(0, keepdim=True))
    for _i in range(N):
        o_cls = random.randint(0, n_class-1)
        while lbl[_i] == o_cls:
            o_cls = random.randint(0, n_class-1)
        cur = ft[_i].unsqueeze(0)
        this_center = c_center[lbl[_i]]
        other_center = c_center[o_cls]
        tcl = F.pairwise_distance(cur, this_center) + m - F.pairwise_distance(cur, other_center)
        loss += tcl if tcl > 0 else 0
        # print((tcl))
    return loss/N
...
def train(data_loader, net, criterion, optimizer, epoch):
    ...
    for i, (lbl, sample) in enumerate(data_loader):
        ...
        optimizer.zero_grad()
        out, g_ft = net(sample, global_ft=True)
        loss_c = criterion(out, lbl)
        loss_tcl = tri_cen_loss(g_ft, lbl)
        loss = loss_c + lam * loss_tcl
...
@hydra.main(config_path=".", config_name="mv_tcl")
